...
public class deletecomp extends JFrame implements ActionListener,ItemListener{
JLabel jl1,jl2;
JComboBox jtf1;
...
JLabel jl1=new JLabel("Delete Company");
...
Connection con=DriverManager.getConnection("jdbc:odbc:MSMS","","");
Statement st=con.createStatement();
ResultSet rs=st.executeQuery("select * from Company WHERE CompanyName="+jtf1);
st.close();
rs.close();
}
catch(Exception e){
...
Class.forName("sun.jdbc.odbc.JdbcOdbcDriver");
Connection con=DriverManager.getConnection("jdbc:odbc:MSMS","","");
String st="DELETE FROM Company WHERE CompanyName=?";
...
{
deletecomp dm=new deletecomp("DELETE COMPANY");
dm.setTitle("DELETE COMPANY");
